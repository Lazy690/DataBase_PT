...
Executer::IndexHeader Iheader{0x44415441, 2};

Executer::struct RecordBankHeader {
    uint32_t MAGIC;
    uint32_t VERSION;
    uint32_t ID_INDEX = 0;
};
Executer::struct IndexHeader {
    uint32_t MAGIC;
    uint32_t VERSION;
    //This also has a dataType assiciated with it.
};

Executer::enum class DataType : uint8_t {
    INTEIRO = 1, //int
    TEXTO = 2, //string
    REAL = 3 //double
};

Executer::struct Column {
    DataType type;
    string name;  
};
Executer::struct Row {
...
Executer::bool save_DBheader(ostream& file, const RecordBankHeader& header) {
    file.write(reinterpret_cast<const char*>(&header), sizeof(RecordBankHeader));
    return file.good();
}
Executer::bool save_Iheader(ostream& file, const IndexHeader& header, const uint32_t& type) {
    file.write(reinterpret_cast<const char*>(&header), sizeof(IndexHeader));
    file.write(reinterpret_cast<const char*>(&type), sizeof(type));
    return file.good();
}

Executer::bool save_MDheader(ostream& file, const MetaDataHeader& header) {
    file.write(reinterpret_cast<const char*>(&header), sizeof(MetaDataHeader));
    return file.good();
}

Executer::bool validate_RecordBank_header(fstream& file) {
...
Executer::bool validate_Index_header(fstream& file, IndexHeader& validate) {
...
Execute::bool save_ID_record(fs::path& path,
                    const uint32_t& id,  
...
Execute::bool save_value_index_record(fs::path path,const Row& row, uint32_t& hold_offset) {
...
Execute::bool fetch_ID_offset(fstream& file, const int ID, int& fetched_offset) {
...
Execute::bool fetch_Value_offset(fstream& file, variant<int32_t, string, double>& value, vector<uint32_t>& fetched_offsets) {
...
Execute::bool append(std::fstream& file, const Row& row, 
...
Execute::bool fetch(istream& file, Row& row, int fetched_offset = -1) {
...
Execute::bool deleteRow(fstream& file, int fetched_offset = -1) {
...
Execute::bool append_updated_row(std::fstream& file, const Row& row, int& hold_new_offset, 
                        int old_index) {
...
Execute::void INSERT(Paths& path, const Row& row) {
...
Execute::void SELECT_byID(Paths& path, int id, Row& row, bool returns_offset = false, int* returned_offset = 0) {  
...
Execute::void SELECT_byValue(Paths& path, string& column, variant<int32_t, string, double>& value, vector<Row>& results,
...
Execute::void DELETE_byID(Paths path, const int id) {
...
Execute::void DELETE_byValue(Paths& path, string& column, variant<int32_t, string, double>& value) {
...
Execute::void UPDATE_byID(Paths& path, int id, map<string, variant<int32_t, string, double>>& values_map) {
...
Execute::void UPDATE_byValue(Paths path, string column_to_search, variant<int32_t, string, double>& value_to_search, 
...
Execute::void print_row_test(vector<Row> results) {
    int count = 0;

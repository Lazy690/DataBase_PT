...
bool append_new_key(fstream& file, const int32_t& item, const uint32_t& offset_db) {
...
bool navigate_tree(fstream& file, const int32_t& item, const int32_t& offset_db) {
    return true;
}
bool insert_into_BST(fstream& file, const int32_t& item, const int32_t& offset_db) {
    ...
    if (file.tellg() == end_of_file ) {
        ...
        if(!append_new_key(file, item, offset_db)) {
...
bool fetch_from_BST(fstream& file, const int32_t& item, int32_t& offset_db) {
    ...
    while(true) {
        ...
        if(key == item) {
            ...
            cout << "Offset in DataBank: " << offset_db << endl;
            return file.good();
...
void test_storage() {
    ...
    vector<int32_t> keys {100, 69, 200, 12, 19, 800};
    vector<uint32_t> offsets = {500, 1000, 1500, 2000, 2500, 3000};
    
    for (int i = 0; i < keys.size(); i++) {
        if(!insert_into_BST(in, keys[i], offsets[i])) {
    ...
    for(auto key: keys) {
        ...
        if(!fetch_from_BST(out, key, fetched_offset)) {
